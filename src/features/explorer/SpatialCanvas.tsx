  ...
  return (
    <section className="spatial" aria-labelledby="spatial-title">
      <div className="spatial-head">
        <div>
          <span className="eyebrow">Coordinate-accurate spatial plane</span>
          <h1 id="spatial-title">Twelve places in the city rhythm</h1>
        </div>
        <div className="spatial-legend">
          <span><i className="dot member" />Pulse member</span>
          <span><i className="ring" />Selected</span>
          <span>Ring size = participation in filtered Pulses, not pedestrian volume</span>
        </div>
      </div>
      <svg viewBox="0 0 760 470" role="img" aria-labelledby="spatial-svg-title spatial-svg-desc">
        <title id="spatial-svg-title">Representative pedestrian sensor locations</title>
        <desc id="spatial-svg-desc">
          A longitude and latitude projection of twelve real sensor locations. Two near-coincident sensors are visually separated with connector lines. No street or boundary map is shown.
        </desc>
              ...
              >
                ...
                <foreignObject x="-18" y="-18" width="36" height="36">
                  <button
                    ...
                    aria-label={`${sensor.locationName}, sensor ${sensor.sensorId}, ${(sensor.coverageRate * 100).toFixed(2)} percent coverage, participates in ${count} filtered Pulses`}
                  />
                </foreignObject>
